...
FileMySD SDClass::open(const char *filepath, uint8_t mode) {
  /*
...
FileMySD FileMySD::openNextFile(uint8_t mode) {
  dir_t p;
...
void FileMySD::rewindDirectory(void) {
  if (isDirectory())
    _file->rewind();
}
