...
class FileMySD : public Stream {
 private:
  char _name[13]; // our name
  SdFile *_file;  // underlying file pointer

public:
  FileMySD(SdFile f, const char *name);     // wraps an underlying SdFile
  FileMySD(void);      // 'empty' constructor
  ~FileMySD(void);     // destructor
#if ARDUINO >= 100
  ...
  FileMySD openNextFile(uint8_t mode = F_RDONLY);
...
public:
  ...
  FileMySD open(const char *filename, uint8_t mode = FILE_READ);
