...
void nb_send(void *pvParameters) {
  ...
  while (1) {
    if (millis() - lastMessage > MIN_SEND_TIME_THRESHOLD) {
        ...
        wifiHashSensor["sensor"] = String(componentName) + String(wifiHashSensorName);
        wifiCountSensor["sensor"] = String(componentName) + String(wifiCountSensorName);
        bleHashSensor["sensor"] = String(componentName) + String(bleHashSensorName);
        bleCountSensor["sensor"] = String(componentName) + String(bleCountSensorName);
        ...
        while (xQueueReceive(NbSendQueue, &SendBuffer, portMAX_DELAY) == pdTRUE || msgCounter >= MAX_NB_MESSAGES) {
          msgCounter++;
